...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('playlists.index') }}" class="text-indigo-400 hover:text-indigo-600">Volver a Playlists</a>
    </div>
    ...
    <div class="mb-6">
        ...
        @if ($playlist->songs->isEmpty())
            <p class="text-gray-400">No hay canciones en esta playlist.</p>
        @else
                ...
                @foreach ($playlist->songs as $index => $song)
                    <div class="bg-gray-800 rounded-lg shadow-lg overflow-hidden transform transition-all hover:scale-105 hover:shadow-xl">
                        ...
                        <div class="p-4 flex justify-between items-center">
                            ...
                            <form action="{{ route('playlists.songs.destroy', [$playlist, $song]) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-red-400 hover:text-red-600 text-sm" onclick="return confirm('¿Eliminar esta canción de la playlist?');">
                                    Eliminar
                                </button>
                            </form>
                        </div>
    ...
    <form action="{{ route('playlists.songs.store', $playlist) }}" method="POST" class="bg-gray-800 rounded-lg p-6 max-w-md">
        @csrf
        <div class="mb-4">
            <label for="song_id" class="block text-gray-300 font-medium mb-2">Añadir Canción</label>
            <select name="song_id" id="song_id" class="w-full bg-gray-700 text-white rounded-lg p-2" required>
                <option value="">Selecciona una canción</option>
                @foreach ($songs as $song)
                    <option value="{{ $song->id }}">{{ $song->title }} - {{ $song->artist->name ?? 'Desconocido' }}</option>
                @endforeach
            </select>
            @error('song_id')
